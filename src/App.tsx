...
export default function App() {
  ...
  if (!userId) return <main className="app-shell login-shell">{message && <NoticeToast message={message} onClose={() => setMessage(null)} />}<header className="topbar"><h1>Fulbo<em>Parejo</em></h1><ThemeSelector preference={themePreference} onChange={changeThemePreference} /></header><section className="panel login-panel"><p className="eyebrow">PLANTEL PRIVADO</p><h2>Tu convocatoria empieza acá.</h2><p className="muted">Entrá con tu cuenta para guardar medias, resultados y tu plantel.</p><button className="google-login" onClick={() => void login()}><span>G</span>Continuar con Google <b>→</b></button></section></main>
