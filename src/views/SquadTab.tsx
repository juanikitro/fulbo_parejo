...
type SortBy = 'rating' | 'name' | 'position' | 'offset' | 'goals'
const emptyGoalTotals = new Map<string, number>()
...
type SquadTabProps = {
  ...
  playerGoalTotals?: ReadonlyMap<string, number>
  onCreatePlayer: () => void
...
function positionsDescription(player: Player) {
  if (!player.preferredPosition) return 'Sin posición'
  const primary = `Primaria: ${player.preferredPosition} · ${positionLabel[player.preferredPosition]}`
  return player.secondaryPosition ? `${primary} · Secundaria: ${player.secondaryPosition} · ${positionLabel[player.secondaryPosition]}` : primary
}

export default function SquadTab({ editable, activePlayers, archivedPlayers, latestOffsets, playerGoalTotals = emptyGoalTotals, onCreatePlayer, onOpenPlayerDetail, onEditPlayer, onArchivePlayer, onRestorePlayer, archivingPlayerId, onExport }: SquadTabProps) {
  ...
  const visiblePlayers = useMemo(() => {
    ...
    return [...activePlayers]
      ...
      .sort((a, b) => {
        ...
        if (sortBy === 'goals') return (playerGoalTotals.get(b.id) ?? 0) - (playerGoalTotals.get(a.id) ?? 0) || byName(a, b)
        return operationalRating(b) - operationalRating(a) || byName(a, b)
      })
  }, [activePlayers, latestOffsets, playerGoalTotals, search, sortBy])

  return <section className="panel"><div className="panel-heading"><div><p className="eyebrow">TU PLANTEL</p><h2>{activePlayers.length} jugadores activos</h2></div>{editable && <button onClick={onCreatePlayer}>+ Crear jugador</button>}</div><div className="squad-controls"><label className="squad-control"><span>Buscar jugador</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nombre…" /></label><label className="squad-control"><span>Ordenar por</span><select value={sortBy} onChange={(event) => setSortBy(event.target.value as SortBy)}><option value="rating">Puntaje</option><option value="name">Nombre</option><option value="position">Posición</option><option value="offset">Buff/debuff</option><option value="goals">Goleadores</option></select></label></div>{visiblePlayers.length ? <div className="squad-grid">{visiblePlayers.map((player) => <article className="player-card" key={player.id}><button type="button" className="player-detail-trigger" aria-label={`Ver ficha de ${player.name}`} onClick={() => void onOpenPlayerDetail(player)}><span className="player-icon" style={{ backgroundColor: player.color }}>{player.icon}</span><span><strong>{player.name}</strong><small>{positionsDescription(player)}</small></span></button><b>{player.baseRating} <small>({player.learnedRating.toFixed(2)})</small></b><small className="goal-total" aria-label={playerGoalTotals.has(player.id) ? `${player.name}: ${playerGoalTotals.get(player.id)} goles registrados` : `${player.name}: sin dato de goles`}>⚽ {playerGoalTotals.get(player.id) ?? '—'}</small><OffsetIndicator offset={latestOffsets.get(player.id)} />{editable && <div className="player-actions"><button aria-label={`Editar ${player.name}`} onClick={() => onEditPlayer(player)}>✏️</button><button aria-label={`Archivar a ${player.name}`} disabled={archivingPlayerId === player.id} onClick={() => void onArchivePlayer(player)}>🗑️</button></div>}</article>)}</div> : <p className="muted squad-empty">No encontramos jugadores con ese nombre.</p>}<details className="trash" aria-label={`Papelera con ${archivedPlayers.length} jugadores archivados`}><summary>Papelera ({archivedPlayers.length})</summary><p className="sr-only" aria-live="polite">{archivedPlayers.length} jugadores archivados.</p>{archivedPlayers.length ? <div className="trash-list">{archivedPlayers.map((player) => <article className="trash-player" key={player.id}><span className="player-icon" style={{ backgroundColor: player.color }}>{player.icon}</span><span><strong>{player.name}</strong><small>{positionsDescription(player)}</small></span>{editable && <button type="button" className="restore-player-button" disabled={archivingPlayerId === player.id} aria-busy={archivingPlayerId === player.id} onClick={() => void onRestorePlayer(player)}>{archivingPlayerId === player.id ? 'Restaurando…' : 'Restaurar'}</button>}</article>)}</div> : <p className="muted trash-empty">No hay jugadores archivados.</p>}</details><div className="roster-export"><p className="muted">Respaldo manual del plantel.</p><button className="export-button" onClick={onExport}>↓ Exportar JSON</button></div></section>
}
