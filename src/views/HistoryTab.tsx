...
export default function HistoryTab({ editable, history, saving, hasMore, loadingMore, onEdit, onDelete, onLoadMore }: HistoryTabProps) {
  return <section className="panel"><p className="eyebrow">HISTORIAL</p>{history.length ? <><RosterStatistics history={history} /><h2 className="history-list-title">Últimos partidos</h2><ol className="history">{history.map((entry) => <li key={entry.id}><div><span>{new Date(entry.createdAt).toLocaleDateString('es-AR')} · {entry.outcome === 'draw' ? 'Empate' : entry.outcome === 'team_one' ? 'Ganó Claro' : 'Ganó Oscuro'}{entry.goalDifference !== null ? ` · Δ ${entry.goalDifference}` : ''}</span>{entry.outcome === 'draw' ? <OffsetPills offsets={entry.playerOffsets} label="Empate" /> : <><OffsetPills offsets={entry.playerOffsets.filter(({ offset }) => offset >= 0)} label="Ganadores" /><OffsetPills offsets={entry.playerOffsets.filter(({ offset }) => offset < 0)} label="Perdedores" /></>}</div><div className="history-actions" hidden={!editable}><button disabled={saving} onClick={() => onEdit(entry)}>✏️</button><button disabled={saving} onClick={() => void onDelete(entry)}>🗑️</button></div></li>)}</ol>{hasMore && <button className="history-more-button" disabled={loadingMore} onClick={() => void onLoadMore()}>{loadingMore ? 'Cargando…' : 'Ver partidos anteriores'}</button>}</> : <><h2>Últimos partidos</h2><p className="muted">Todavía no registraste resultados.</p></>}</section>
}
