alter table public.match_participants
  add column rating_offset numeric(12, 8) not null default 0;
...
begin
  ...
  loop
    ...
    update public.match_participants set rating_offset = case when team = 1 then v_delta else -v_delta end where match_id = v_match.id;
    update public.players set learned_rating = learned_rating + case when match_participants.team = 1 then v_delta else -v_delta end
    from public.match_participants where match_participants.match_id = v_match.id and players.id = match_participants.player_id;
  end loop;
end;
$$;
...
begin
  for v_match in
    select distinct on (matches.roster_id) matches.id, match_results.outcome, match_results.goal_difference
    from public.matches join public.match_results on match_results.match_id = matches.id
    where matches.status = 'completed'
    order by matches.roster_id, matches.created_at desc, matches.id desc
  loop
